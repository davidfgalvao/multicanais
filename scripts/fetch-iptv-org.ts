...
async function main() {
  ...
  const url = process.env.IPTV_PLAYLIST_URL?.trim() || DEFAULT_URL;
  console.log("A obter:", url);
  const res = await fetch(url, {
    redirect: "follow",
    headers: { "User-Agent": "multicanais-esporte/1.0 (playlist fetch)" },
  });
  if (!res.ok) {
    throw new Error(`HTTP ${res.status} ao obter playlist: ${url}`);
  }
  ...
  console.log(`Total ${withIds.length} canais → ${outPath}`);
}
